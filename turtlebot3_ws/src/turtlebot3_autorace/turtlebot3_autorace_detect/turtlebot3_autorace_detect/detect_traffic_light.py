...
class DetectTrafficLight(Node):
    ...
    def __init__(self):
        ...
        self.traffic_color_pub = self.create_publisher(String, '/traffic_light_color', 10)
    ...
    def find_traffic_light(self):
        ...
        if detected_color and detected_color != self.last_detected_color:
            self.traffic_color_pub.publish(String(data=detected_color))
            self.get_logger().info(f'🚦 감지된 신호등 색상 publish: {detected_color}')
